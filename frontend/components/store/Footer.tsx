...
export default function Footer() {
  ...
  const categoryTree = useMemo<Category[]>(() => {
    ...
    const activeCategories = uniqueCategories(
      allCategories.filter((category) => category.status !== "inactive"),
    );

    const hasNestedChildren = activeCategories.some(
      (category) =>
    ...
    if (hasNestedChildren) {
      return activeCategories
        ...
        .map((parent) => {
          const children = uniqueCategories(
      ...
      <div className="relative">
          ...
          <div className="grid gap-12 lg:grid-cols-[1.2fr_2fr_0.9fr_0.9fr]">
            ...
            <div>
              ...
              <p className="mt-6 max-w-sm text-sm leading-7 text-gray-400">
                Discover timeless bangles, elegant designs and beautiful
                accessories made to celebrate your everyday and special moments.
              </p>
            ...
            <FooterColumn eyebrow="Support" title="Customer Care">
            ...
            <FooterColumn eyebrow="Company" title="BanglesMart">
        ...
        <section className="border-b border-white/10">
          <div className="mx-auto grid max-w-7xl gap-4 px-5 py-6 sm:grid-cols-3 sm:px-6 lg:px-8">
            <ContactItem
              icon={<Mail size={17} />}
              label="Email Support"
              value="banglesmart.business@gmail.com"
            />
        ...
        <section className="mx-auto max-w-7xl px-5 py-7 sm:px-6 lg:px-8">
          <div className="flex flex-col gap-5 text-xs text-gray-500 sm:flex-row sm:items-center sm:justify-between">
            <p>
              © {new Date().getFullYear()} BanglesMart. All rights reserved.
            </p>

            <div className="flex flex-wrap items-center gap-2">
              <span className="inline-flex items-center gap-1.5">
                Made with
                <Heart size={13} className="fill-[#8f0828] text-[#8f0828]" />
                in India
              </span>

              <span className="hidden text-white/20 sm:inline">•</span>

              <span>Premium jewellery, beautifully delivered.</span>
            </div>
          </div>
        </section>
      </div>
...
function FooterColumn({
  ...
  return (
    <div>
      ...
      <div className="mt-6 flex flex-col gap-3.5 text-sm">{children}</div>
    </div>
  );
}
