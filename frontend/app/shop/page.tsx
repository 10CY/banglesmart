...
function ShopPageContent() {
  ...
  const categories: ShopCategory[] = Array.isArray(catalog?.categories)
  ...
  const urlNewArrival = searchParams.get("new_arrival") === "1";

  const urlBestSeller = searchParams.get("best_seller") === "1";
  ...
  const [newArrival, setNewArrival] = useState(urlNewArrival);

  const [bestSeller, setBestSeller] = useState(urlBestSeller);
  ...
  useEffect(() => {
    ...
    async function loadProducts() {
      ...
      try {
        ...
        const productList = Array.isArray(paginator?.data)
          ? paginator.data
        ...
        setPagination({
          current_page: Number(paginator?.current_page || page),

          last_page: Number(paginator?.last_page || 1),

          total: Number(paginator?.total || productList.length),

          per_page: Number(paginator?.per_page || 12),
        });
      } catch (error) {
  ...
  }, [category, sort, minPrice, maxPrice, newArrival, bestSeller, featured]);
  ...
  const filterCategories = useMemo<FilterCategory[]>(() => {
    return categories.filter(Boolean).map((parent) => ({
      id: Number(parent.id),
  ...
  const currentCategoryName = useMemo(() => {
    ...
    for (const parent of filterCategories) {
      ...
      <section className="border-b border-[#eee8de] bg-[#f5efe5]">
        <div className="mx-auto max-w-7xl px-5 py-12 sm:px-6 lg:px-8 lg:py-16">
          ...
          <p className="mt-4 max-w-2xl text-sm leading-7 text-[#6d675e]">
            Discover our curated collection of elegant bangles, bridal designs,
            traditional styles and contemporary pieces crafted for every
            occasion.
          </p>
        </div>
      </section>
              ...
              <div className="flex min-h-[420px] items-center justify-center rounded-3xl border border-dashed border-[#d9d0c2] bg-white px-6 text-center">
                <div>
                  ...
                  <p className="mx-auto mt-2 max-w-md text-sm leading-6 text-[#777]">
                    Try another keyword, adjust the price range, or clear your
                    filters to explore the complete collection.
                  </p>
