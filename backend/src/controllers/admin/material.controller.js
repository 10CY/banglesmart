...
export async function index(req, res) {
  try {
    const rows = await query(
      `SELECT id,name,status,created_at,updated_at FROM materials ORDER BY id DESC`,
    );
...
export async function store(req, res) {
  ...
  if (!["active", "inactive"].includes(status))
    return fail(res, "Invalid material status.", 422);
  try {
    const existing = (
      await query(`SELECT id FROM materials WHERE name=? LIMIT 1`, [name])
    )[0];
    if (existing)
      return fail(res, "A material with this name already exists.", 422);
    const result = await query(
      `INSERT INTO materials (name,status,created_at,updated_at) VALUES (?,?,NOW(),NOW())`,
      [name, status],
    );
    await audit(req, "material_created", "material", result.insertId, {
      name,
      status,
    });
    return ok(
      res,
      {
        ...
        data: (
          await query(`SELECT * FROM materials WHERE id=?`, [result.insertId])
        )[0],
      },
      201,
    );
...
export async function update(req, res) {
  ...
  try {
    ...
    const duplicate = (
      await query(`SELECT id FROM materials WHERE name=? AND id<>? LIMIT 1`, [
        name,
        id,
      ])
    )[0];
    if (duplicate)
      return fail(res, "A material with this name already exists.", 422);
    await query(
      `UPDATE materials SET name=?,status=?,updated_at=NOW() WHERE id=?`,
      [name, status, id],
    );
    ...
    return ok(res, {
      ...
      data: (await query(`SELECT * FROM materials WHERE id=?`, [id]))[0],
    });
  } catch (error) {
